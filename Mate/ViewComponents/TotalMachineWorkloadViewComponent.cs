        ...
        public async Task<IViewComponentResult> InvokeAsync(string machine)
        {
            var generateChartTask = Task.Run(function: () =>
            {
               ...
               Chart chart = new Chart();
                ...
                var endSum = _context.SimulationJobs.Where(predicate: x => x.CapabilityProvider == machine).Sum(selector: x => x.End.ToFileTimeUtc());
                var startSum = _context.SimulationJobs.Where(predicate: x => x.CapabilityProvider == machine).Sum(selector: x => x.Start.ToFileTimeUtc());
                var max = _context.SimulationJobs.Max(selector: x => x.End.ToFileTimeUtc());
