    ...
    public class JobPriorityRules : TestKit
    {
        ...
        public async Task FBucketPriority()
        {
            await Task.Run(action: () =>
            {
                ...
                var w1 = MessageFactory.ToOperationItem(new M_Operation() { Duration = 10 , ResourceTool = new M_ResourceTool { Name = "Saw blade big"} }, 50, productionAgent: probe, firstOperation: false ,  0);
                var w2 = MessageFactory.ToOperationItem(new M_Operation() { Duration = 5 , ResourceTool = new M_ResourceTool { Name = "Saw blade big" } }, 20, productionAgent: probe, firstOperation: false, 0);
                var w3 = MessageFactory.ToOperationItem(new M_Operation() { Duration = 15 , ResourceTool = new M_ResourceTool { Name = "Saw blade big" } }, 100, productionAgent: probe, firstOperation: false, 0);
