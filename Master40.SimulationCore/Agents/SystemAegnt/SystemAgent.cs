...
{
    public partial class SystemAgent : Agent
    {
        ...
        // public Constructor
        public static Props Props(ActorPaths actorPaths, long time, bool debug, ProductionDomainContext productionDomainContext, IMessageHub messageHub, SimulationConfiguration simConfig)
        {
            return Akka.Actor.Props.Create(() => new SystemAgent(actorPaths, time, debug, productionDomainContext, messageHub, simConfig));
        }

        public SystemAgent(ActorPaths actorPaths, long time, bool debug, ProductionDomainContext productionDomainContext, IMessageHub messageHub, SimulationConfiguration simConfig) 
            : base(actorPaths, time, debug)
        {
            _productionDomainContext = productionDomainContext;
            _messageHub = messageHub;
            _simConfig = simConfig;
            CreateAndEnqueue(Instruction.PopOrder.Create("Pop", ActorPaths.SystemAgent.Ref), 1);
            CreateAndEnqueue(Instruction.EndSimulation.Create(null, ActorPaths.SystemAgent.Ref), 21000);
        }
        ...
        private void CreateContractAgent(Instruction.CreateContractAgent instruction)
        {
            ...
            var contract = UntypedActor.Context.ActorOf(props: ContractAgent.Props(actorPaths: ActorPaths
                                                                     , time: TimePeriod
                                                                     , debug: true)
                                           ,name: "Contract(" + orderPart.Id + ")");

            // Start Order
            CreateAndEnqueue(ContractAgent.Instruction.StartOrder.Create(orderPart, contract));
        }
        ...
        private void RequestArticleBom(RequestItem requestItem)
        {
            ...
            // calback with po.bom
            CreateAndEnqueue(DispoAgent.Instruction.ResponseFromSystemForBom.Create(article, Sender));                        
        }
        ...
        private void PopOrder()
        {
                                                                ...
                                                                .AsNoTracking())
            {
                ...
                {
                    CreateAndEnqueue(Instruction.CreateContractAgent.Create(orderpart, Self));
                }
                else
                {
                    long period = orderpart.Order.DueTime - (10 * 60); // 540
                    if (period < 0) { period = 0; }
                    CreateAndEnqueue(instruction: Instruction.CreateContractAgent.Create(orderpart, Self), waitFor: period);
                }
            }
