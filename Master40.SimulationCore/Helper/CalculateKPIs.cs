...
namespace Master40.SimulationCore.Helper
{
    public static class CalculateKpis
    {
        ...
        public static void CalculateAllKpis(ProductionDomainContext context
            ...
            , int time = 0)
        {
            CalculateLeadTime(context, resultContext, simulationId,  simulationType,  simulationNumber, final, time);
            CalculateMachineUtilization(context, resultContext, simulationId,  simulationType,  simulationNumber, final, time);
            CalculateTimeliness(context, resultContext, simulationId,  simulationType,  simulationNumber, final, time);
            ArticleStockEvolution(context, resultContext, simulationId, simulationType, simulationNumber, final, time);
            CalculateLayTimes(context, resultContext, simulationId, simulationType, simulationNumber, final, time);
        }

        public static void CalculateLayTimes(ProductionDomainContext context, ResultContext resultContext, int simulationId,
            SimulationType simulationType, int simulationNumber, bool final, int time)
        {
            var simConfig = resultContext.SimulationConfigurations.Single(a => a.Id == simulationId);
            ...
            foreach (var article in articles) //.Where(x => x.Name.Equals("Dump-Truck")))
            {
                ...
                resultContext.Kpis.Add(new Kpi()
                {
                    Name = article.Name,
        ...
        /// <param name="final"></param>
        /// <param name="time"></param>
        public static void CalculateLeadTime(ProductionDomainContext context, ResultContext resultContext, int simulationId,
            SimulationType simulationType, int simulationNumber, bool final, int time)
        {
            var simConfig = resultContext.SimulationConfigurations.Single(a => a.Id == simulationId);
            //calculate lead times for each product
            var finishedProducts = final
                ? resultContext.SimulationOperations
                    .Where(x => x.SimulationConfigurationId == simulationId && x.SimulationType == simulationType)
            ...
            var simulationWorkSchedules = resultContext.SimulationOperations.Where(
            ...
            {
                var start = context.GetEarliestStart(resultContext, product, simulationType, simulationId, simulationWorkSchedules);
            ...
            resultContext.Kpis.AddRange(leadTimesBoxPlot);
            resultContext.SaveChanges();
        }
        ...
        /// <param name="final"></param>
        /// <param name="time"></param>
        public static void CalculateMeanStartTime(ProductionDomainContext context, ResultContext resultContext, int simulationId,
            SimulationType simulationType, int simulationNumber, bool final, int time)
        {
            var simConfig = resultContext.SimulationConfigurations.Single(a => a.Id == simulationId);
            //calculate lead times for each product
            ...
            var simOrders = resultContext.SimulationOrders.Where(
        ...
        /// <param name="final"></param>
        /// <param name="time"></param>
        public static void CalculateMachineUtilization(ProductionDomainContext context, ResultContext resultContext, int simulationId,
            SimulationType simulationType, int simulationNumber, bool final, int time)
        {
            var simConfig = resultContext.SimulationConfigurations.Single(a => a.Id == simulationId);
            ...
            //get working time
            var content = final
                ? resultContext.SimulationOperations.Where(a => a.Start >= simConfig.SettlingStart
                ...
                : resultContext.SimulationOperations.Where(a => a.Start >= time - simConfig.DynamicKpiTimeSpan
                                                                && a.End <= time
                                                                && a.SimulationNumber == simulationNumber
                                                                && a.SimulationType == simulationType
                                                                && a.SimulationConfigurationId == simulationId)
            ...
            resultContext.Kpis.AddRange(kpis);
            resultContext.SaveChanges();

            if (!final)
            { return; }

            var allKpis = resultContext.Kpis.Where(a => a.KpiType == KpiType.MachineUtilization
                                                    && a.SimulationConfigurationId == simulationId
                                                    && a.SimulationNumber == simulationNumber
                                                    && a.SimulationType == simulationType
                                                    && !a.IsFinal
            ...
            resultContext.UpdateRange(kpis);
            resultContext.SaveChanges();
        }
        ...
        /// <param name="final"></param>
        /// <param name="time"></param>
        public static void CalculateTimeliness(ProductionDomainContext context, ResultContext resultContext, int simulationId,
            SimulationType simulationType, int simulationNumber, bool final, int time)
        {
            var simConfig = resultContext.SimulationConfigurations.Single(a => a.Id == simulationId);
            ...
            */
            var orderTimeliness = final ?
                //then
                resultContext.SimulationOrders
                .Where(x => x.SimulationConfigurationId == simulationId && x.SimulationType == simulationType)
                .Where(a =>
        ...
        /// <param name="final"></param>
        /// <param name="time"></param>
        public static void ArticleStockEvolution(ProductionDomainContext context, ResultContext resultContext, int simulationId,
            SimulationType simulationType, int simulationNumber, bool final, int time)
        {
            var simConfig = resultContext.SimulationConfigurations.Single(a => a.Id == simulationId);
        ...
        /// <param name="simulationType"></param>
        /// <param name="simulationNumber"></param>
        public static void MachineSattleTime(ProductionDomainContext context, ResultContext resultContext, SimulationConfiguration simConfig,
            SimulationType simulationType, int simulationNumber)
        {
            ...
            {
                CalculateMachineUtilization(context, resultContext, simConfig.Id, simulationType, simulationNumber, false, i);
            }
        }

        public static Task ConsolidateRuns(ResultContext resultContext, int simulationId)
        {
            var task = Task.Run(() =>
            {
                ConsolidateMachineWorkload(resultContext, simulationId, SimulationType.Decentral);
                ConsolidateLeadTime(resultContext, simulationId, SimulationType.Decentral);
                ConsolidateTimeliness(resultContext, simulationId, SimulationType.Decentral);
                ConsolidateLayTime(resultContext, simulationId, SimulationType.Decentral);
            });
            return task;
        }

        private static void ConsolidateMachineWorkload(ResultContext resultContext, int simulationId,
            SimulationType simType)
        {

            var kpis = resultContext.Kpis.Where(x => x.SimulationConfigurationId == simulationId
            ...
            resultContext.Kpis.AddRange(summary);
            resultContext.SaveChanges();
        }

        private static void ConsolidateTimeliness(ResultContext resultContext, int simulationId, SimulationType simType)
        {
        ...
        private static void ConsolidateLeadTime(ResultContext resultContext, int simulationId, SimulationType simType)
        {
            var kpis = resultContext.Kpis.Where(x => x.SimulationConfigurationId == simulationId
                                                && x.SimulationType == simType
        ...
        private static void ConsolidateLayTime(ResultContext resultContext, int simulationId, SimulationType simType)
        {
            var kpis = resultContext.Kpis.Where(x => x.SimulationConfigurationId == simulationId
                                               && x.SimulationType == simType
